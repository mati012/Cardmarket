...
export class RegistrarseComponent implements OnInit {
  ...
  getErrorMessage(controlName: string): string {
    ...
    if (control.hasError('maxlength')) return 'La contraseña no puede tener más de 18 caracteres';
    if (control.hasError('passwordRequirements')) return 'La contraseña debe contener al menos un número y una mayúscula';
    if (control.hasError('minAge')) return 'Debes tener al menos 13 años para registrarte';
    
    return '';
  }

  async onSubmit() {
    this.submitted = true; // Set submitted to true to show validation errors
